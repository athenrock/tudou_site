...
def update_table(table, params, whargs):
    r'''
    自定义包装
    @:params:要更新的参数 必须是dict
    @:arg: 更新条件 也必须是dict
    Execute update SQL.
    >>> params = dict(name='Michael', passwd='123456')
    >>> args =dict(id=1)
    >>> update_table('user',params,args)
    '''
    cols, value = zip(*params.iteritems())

    sql = 'update `%s` set %s' % (
        table, ','.join(["`%s`=?" % col for col in cols]))

    wk, wv = zip(*whargs.iteritems())
    sql = '%s where 1=1 %s' % (sql, ''.join(['and `%s`=? ' % x for x in wk]))
    tup = value + wv

    print sql
    print tup
    return _update(sql, *tup)
